...
export const tonApiRouter = createTRPCRouter({
    ...
    .input(z.object({
        ...
        const { public_key } = await client.accounts.getAccountPublicKey(input.walletInfo.account.address)
        
        const pubKey = Buffer.from(public_key, 'hex')
          
        const parsedMessage = ConvertTonProofMessage(input.walletInfo as Wallet, proof as TonProofItemReplySuccess)
